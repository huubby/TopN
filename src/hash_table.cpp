        ...
        destroy_func key_destroy_func, destroy_func value_destroy_func)
{
    ...
    if (hash_func == NULL || hash_func == NULL) {
        ...
        const void *key, uint *hash_return)
{
    ...
    hash_value = hash_table->hfunc(key);
        ...
        bool reusing_key)
{
    ...
    if (hash_table->keys == hash_table->values && key != value)
        hash_table->values = (void**)memdup(hash_table->keys,
...
void hash_table_foreach(hash_table_t *hash_table, traverse_pair_func func,
        void *user_data)
{
    int i;
